...
export default function DomainOverlay({ title, tagline, projects, onClose }) {
  ...
  // Lightbox slider : { items: [...media], index: number } ou null
  ...
  // ESC / flèches clavier pour la lightbox
  useEffect(() => {
    const handleKey = (e) => {
      if (!lightbox) return;
      if (e.key === "Escape") {
  ...
  return (
    <div className="fixed inset-0 z-30 flex items-center justify-center px-4 pointer-events-none">
      ...
      <div className="relative max-w-4xl w-full pointer-events-auto">
        <div
          className={
            "overlay-card p-6 md:p-8 flex flex-col gap-5 " +
        ...
        >
          ...
          {/* Liste de projets */}
          <div className="space-y-3 md:space-y-4 mt-1">
                            ...
                            <div className="mt-3 flex flex-wrap gap-3 md:gap-4">
                              {project.media.map((item, index) => (
                                <button
                                  ...
                                  type="button"
                                  onClick={(e) => {
                                    e.stopPropagation(); // évite de replier la carte
                                    setLightbox({
                                      items: project.media,
                                      index,
                                    });
                                  }}
                                  className="group flex flex-col gap-1 w-24 md:w-32 text-left"
                                >
                                  <div
                                    className="
                                      relative 
                                      ...
                                      group-hover:shadow-[0_0_26px_-6px_rgba(56,189,248,0.6)]
                                    "
                                  >
                                    {item.type === "image" && (
                                      <img
